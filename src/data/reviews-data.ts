
export interface eachReview {
    placeId: string,
    reviewerName: string,
    reviewRating: "1" | "2" | "3" | "4" | "5",
    reviewerImageLink: string,
    reviewerExperienceInYears: number,
    reviewText: string
    reviewDate: Date
}



export const reviews: eachReview[] = [

    {
        placeId: "01",
        reviewerName: "Subhankar Sarkar",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2010')
    },
    {
        placeId: "01",
        reviewerName: "Prasenjit Das",
        reviewerImageLink: "2.jpeg",
        reviewRating: "3",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2015')


    },
    {
        placeId: "01",
        reviewerName: "Sumitra Das",
        reviewerImageLink: "3.jpeg",
        reviewRating: "3",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/15/2024')


    },
    {
        placeId: "02",
        reviewerName: "Sagar Nandi",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2024')

    },
    {
        placeId: "03",
        reviewerName: "Subhankar Sarkar",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2024')


    },
    {
        placeId: "05",
        reviewerName: "Subhankar Sarkar",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2024')


    },
    {
        placeId: "02",
        reviewerName: "Subhankar Sarkar",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2024')


    },
    {
        placeId: "04",
        reviewerName: "Subhankar Sarkar",
        reviewerImageLink: "1.jpeg",
        reviewRating: "4",
        reviewerExperienceInYears: 2,
        reviewText: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Perferendis doloremque omnis accusamus numquam impedit tenetur nisi repellat? Adipisci, harum? Nemo.",
        reviewDate: new Date('09/24/2024')


    },



]